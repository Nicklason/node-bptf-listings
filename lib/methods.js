...
Listings.prototype._parse = function (item) {
    var schema = this._items.schema.getItem(item.defindex);
    if (schema == null) {
        this.emit('error', item.defindex || '', 'Not a valid / missing defindex');
        return null;
    }

    var quality = this._items.schema.getQuality(item.quality);
    ...
    var name = this._items.schema.getDisplayName(item);
